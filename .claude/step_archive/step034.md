# Step 34 - 레이아웃 스크린샷 검증 (독립 검증 루프)

Step 030에서 구현된 레이아웃이 설계대로 구현되었는지 **작성자와 검증자를 분리하여** 검증한다.
...
- 데스크톱 (1920×1080): `.claude/screenshots/layout-verify-desktop-r1.png`
- 태블릿 (768×1024): `.claude/screenshots/layout-verify-tablet-r1.png`
- 모바일 (390×844): `.claude/screenshots/layout-verify-mobile-r1.png`
...
2. 구현 스크린샷을 모두 Read한다:
   - `.claude/screenshots/layout-verify-desktop-rN.png`
   - `.claude/screenshots/layout-verify-tablet-rN.png`
   - `.claude/screenshots/layout-verify-mobile-rN.png`

3. 이전 라운드 검증 결과가 있으면 모두 Read한다:
   - `.claude/step_archive/outputs/step034_검증_r*.md` (Glob 검색)
...
6. 결과를 `.claude/step_archive/outputs/step034_검증_rN.md`에 저장한다.
...
1. 최신 검증 피드백을 Read한다:
   - `.claude/step_archive/outputs/step034_검증_rN.md`

2. 이전 라운드 수정 내역이 있으면 Read한다:
   - `.claude/step_archive/outputs/step034_수정_r*.md` (Glob 검색)
...
5. 수정 내역을 `.claude/step_archive/outputs/step034_수정_rN.md`에 기록한다.
```
