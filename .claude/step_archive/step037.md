# Step 37 - Awwwards 디자인 검증 및 CSS 보강 (독립 검증 루프)

Step 030에서 구현된 CSS가 Awwwards 조사 결과를 충실히 반영했는지 **작성자와 검증자를 분리하여** 검증하고 보강한다.
...
Playwright로 현재 구현의 스크린샷을 촬영하여 `.claude/screenshots/compare-awwwards-applied-r1.png`에 저장한다.
...
1. 아래 Awwwards 원본 스크린샷을 모두 Read한다:
   - `.claude/screenshots/research/awwwards-*.png` (Glob 검색)
   - `.claude/awwwards-*.txt` (Glob 검색)

2. 구현 스크린샷을 Read한다:
   - `.claude/screenshots/compare-awwwards-applied-rN.png`

3. 이전 라운드 검증 결과가 있으면 모두 Read한다:
   - `.claude/step_archive/outputs/step037_검증_r*.md` (Glob 검색)
...
5. 결과를 `.claude/step_archive/outputs/step037_검증_rN.md`에 저장한다.
...
1. 최신 검증 피드백을 Read한다:
   - `.claude/step_archive/outputs/step037_검증_rN.md`

2. 이전 라운드 수정 내역이 있으면 Read한다:
   - `.claude/step_archive/outputs/step037_수정_r*.md` (Glob 검색)
...
5. 수정 내역을 `.claude/step_archive/outputs/step037_수정_rN.md`에 기록한다.
```
...
수정 후 Playwright로 스크린샷을 `.claude/screenshots/compare-awwwards-applied-rN.png` (N = 다음 라운드 번호)로 재촬영하고, 2단계(에이전트 B 검증)부터 반복한다.
