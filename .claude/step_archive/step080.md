# Step 80 - Playwright 디자인 시각 검증 (독립 검증 루프)
...
1. **프로젝트 UI 특성 파악**
   - step030_설계_chunk*.md (레이아웃 설계)에서 UI 구조/컴포넌트 목록 추출
   - step031_설계_chunk*.md (전체 설계)에서 UI 구조/컴포넌트 목록 추출
...
Playwright 스크립트 실행 → 스크린샷을 `.claude/screenshots/design/` 에 저장
